...
Modbus.prototype.requestReadCoils = function (slaveAddress, startAddress, quantity, callback) {
    ...
    if (this._mode === 'ascii') {
        expectedLength = expectedLength * 2 + 1; // start(1) + addr(1 * 2) + FC(1 * 2) + byteCount(1 * 2) + data (n * 2) + lrc (1 * 2) + end(2)
    }
...
Modbus.prototype.requestWriteSingleCoil = function (slaveAddress, address, state, callback) {
    if (this._mode === 'ascii') {
        this._codec.try(17); // start(1) + addr(1 * 2) + FC(1 * 2) + OutputAddress(2 * 2) + OutputValue(2 * 2) + lrc (1 * 2) + end(2)
    } else {
